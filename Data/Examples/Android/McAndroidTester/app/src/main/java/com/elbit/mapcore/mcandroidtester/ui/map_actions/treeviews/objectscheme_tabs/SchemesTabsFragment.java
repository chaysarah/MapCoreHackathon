...
public class SchemesTabsFragment extends Fragment implements FragmentWithObject {
    ...
    private static final int TABS_NUN = 1;
    ...
    public static SchemesTabsFragment newInstance() {
    ...
    public void initTabs(View rootView) {
        ...
        mViewPager = (ViewPager) rootView.findViewById(R.id.objectscheme_tabs_container);
        mViewPager.setAdapter(mSectionsPagerAdapter);
        mViewPager.setOffscreenPageLimit(TABS_NUN);

        TabLayout tabLayout = (TabLayout) rootView.findViewById(R.id.objectscheme_tabs);
        tabLayout.setupWithViewPager(mViewPager);
    }

    private void setTitle() {
        getActivity().setTitle("object scheme tabs");
    }
    @Override
    public void onHiddenChanged(boolean hidden) {
        ...
        if (mContext instanceof MapsContainerActivity) {
            ((MapsContainerActivity) mContext).mCurFragmentTag = this.getClass().getSimpleName();
        }
    }
    ...
    public void onAttach(Context context) {
        ...
        if (context instanceof MapsContainerActivity) {
            ((MapsContainerActivity) context).mCurFragmentTag = this.getClass().getSimpleName();
        }
    }
    ...
    public class SectionsPagerAdapter extends FragmentPagerAdapter {
        ...
        public Fragment getItem(int position) {
            switch (position) {
                case 0:
                    Fragment schemeGeneralFragment = SchemeGeneralTabFragment.newInstance();
                    ((FragmentWithObject) schemeGeneralFragment).setObject(mcObjectScheme);
                    return schemeGeneralFragment;
               /* case 1:
                    Fragment objectLocationsObject = SchemeGeneralTabFragment.newInstance();
                    ((FragmentWithObject) objectLocationsObject).setObject(mcObjectScheme);

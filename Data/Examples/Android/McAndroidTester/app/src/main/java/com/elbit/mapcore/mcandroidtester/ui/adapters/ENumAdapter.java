...
public class ENumAdapter extends ArrayAdapter {
     ...
     */
    public ENumAdapter(Context context, int resource, List objects) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (getCount() > 0 && (getItem(position) != null)) {
                ((CheckedTextView) convertView.findViewById(R.id.list_view)).setText(((Enum)getItem(position)).name());
        }
        return convertView;
    }
}
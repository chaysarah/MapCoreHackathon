package com.elbit.mapcore.mcandroidtester.utils.treeview.model;
...
import com.elbit.mapcore.mcandroidtester.utils.treeview.view.TreeNodeWrapperView;
...
public class TreeNode {
    ...
    String mFragmentTag;
    Object mImcObj;
    public TreeNode treeNodeToSelect;
    ...
    public TreeNode getNodeByObjectHashCode(TreeNode node, int hashCode) {
        if (treeNodeToSelect == null) {
            for (TreeNode treeNode : node.getChildren()) {
               // if (treeNode.isLeaf()) {
                    Object imcObj=((IconTreeItemHolder.IconTreeItem) treeNode.mValue).mImcObj;
                    if (imcObj!=null&&hashCode == ((IconTreeItemHolder.IconTreeItem) treeNode.mValue).mImcObj.hashCode()) {
                        treeNodeToSelect = treeNode;
                        break;
                    }
                //} else
                if(!treeNode.isLeaf()&&treeNodeToSelect==null)
                    getNodeByObjectHashCode(treeNode, hashCode);
            }
            return treeNodeToSelect;
        } else
            return treeNodeToSelect;
    }
    ...
    public void setSelected(boolean selected) {
        mSelected = selected;
        mViewHolder.getView().setBackgroundColor(selected ? 0xFFC5CAE9 : Color.TRANSPARENT);
    }

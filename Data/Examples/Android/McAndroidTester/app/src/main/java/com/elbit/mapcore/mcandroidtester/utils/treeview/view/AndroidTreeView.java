package com.elbit.mapcore.mcandroidtester.utils.treeview.view;
...
import com.elbit.mapcore.mcandroidtester.utils.treeview.holder.SimpleViewHolder;
...
public class AndroidTreeView {
    ...
    public static final String SELECTED_TREE_VIEW_NODE = "SELECTED_TREE_VIEW_NODE";
    ...
    public void expandNodeTillRoot(final TreeNode node)
    {
    ...
    private void addNode(ViewGroup container, final TreeNode n) {
        ...
        nodeView.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                IconTreeItemHolder.IconTreeItem value = (IconTreeItemHolder.IconTreeItem)n.getValue();
                if(value != null && value.mImcObj != null) {
                    if(value.mTreeViewType == IconTreeItemHolder.IconTreeItem.TreeViewType.OM)
                        OverlayManagerTreeViewFragment.SelectedNodeHashCode = value.mImcObj.hashCode();
                    else if(value.mTreeViewType == IconTreeItemHolder.IconTreeItem.TreeViewType.TL)
                        TerrainLayersTreeViewFragment.SelectedNodeHashCode = value.mImcObj.hashCode();
                    else if(value.mTreeViewType == IconTreeItemHolder.IconTreeItem.TreeViewType.VT)
                        ViewportTerrainTreeViewFragment.SelectedNodeHashCode = value.mImcObj.hashCode();
                }
            }
        });
    ...
    public void removeAllNodes() {
        List<TreeNode> children= new ArrayList<>(mRoot.getChildren());
        for (TreeNode n : children) {
            removeNode(n);
        }
    }

    public void expandNodeTillRoot(int selectedNodeHashCode) {
        if (selectedNodeHashCode != 0) {
            TreeNode node = mRoot.getNodeByObjectHashCode(mRoot, selectedNodeHashCode);
            if (node != null) {
                if (node.getParent() != null && (node.getParent() == mRoot || !node.getParent().isExpanded())) {
                    expandNodeTillRoot(node.getParent());
                }
            }
        }
    }

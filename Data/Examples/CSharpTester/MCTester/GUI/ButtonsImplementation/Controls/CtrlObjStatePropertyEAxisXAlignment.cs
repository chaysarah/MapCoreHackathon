...
{
    public partial class CtrlObjStatePropertyEAxisXAlignment : CtrlObjStatePropertyDataHandlerEAxisXAlignment
    {
        private Type m_RegEnumType = null;
        private Type m_SelEnumType = null;
        private string m_EnumType = null;

        public CtrlObjStatePropertyEAxisXAlignment()
        {
            InitializeComponent();
            m_InitValue = DNEAxisXAlignment._EXA_RIGHT;
            SetEnumList();
        }
        ...
        private void SetEnumList()
        {
            ...
            Type enumType = typeof(DNEAxisXAlignment);
        ...
        public DNEAxisXAlignment RegEnumVal
        {
            get
            {
                if (cmbRegEnum.SelectedItem != null)
                    return (DNEAxisXAlignment)Enum.Parse(m_RegEnumType, cmbRegEnum.SelectedItem.ToString());
                else
                    return 0;
            }
            set
        ...
        public DNEAxisXAlignment SelEnumVal
        {
            get
            {
                if (cmbSelEnum.SelectedItem != null)
                    return (DNEAxisXAlignment)Enum.Parse(m_SelEnumType, cmbSelEnum.SelectedItem.ToString());
                else
                    return 0;
            }
            set
        ...
        public override string GetPropertyEnumType() { return "DNEAxisXAlignment"; }

        public override void SetValue(uint propertyId, object value)
        {
            if (RegPropertyID == propertyId)
                RegEnumVal = (DNEAxisXAlignment)value;
            if (SelPropertyID == propertyId)
                SelEnumVal = (DNEAxisXAlignment)value;
            SetStateValueByProperty(propertyId, (DNEAxisXAlignment)value);
        }
    ...
    public class CtrlObjStatePropertyDataHandlerEAxisXAlignment : CtrlObjStatePropertyDataHandler<DNEAxisXAlignment>
    {

...
{
    public partial class CtrlObjStatePropertyEFillStyle : CtrlObjStatePropertyDataHandlerEFillStyle
    {
        private Type m_RegEnumType = null;
        private Type m_SelEnumType = null;
        private string m_EnumType = null;

        public CtrlObjStatePropertyEFillStyle()
        {
            InitializeComponent();
            m_InitValue = DNEFillStyle._EFS_SOLID;
            SetEnumList();
        }
        ...
        private void SetEnumList()
        {
            ...
            Type enumType = typeof(DNEFillStyle);
        ...
        public DNEFillStyle RegEnumVal
        {
            get
            {
                if (cmbRegEnum.SelectedItem != null)
                    return (DNEFillStyle)Enum.Parse(m_RegEnumType, cmbRegEnum.SelectedItem.ToString());
                else
                    return 0;
            }
            set
        ...
        public DNEFillStyle SelEnumVal
        {
            get
            {
                if (cmbSelEnum.SelectedItem != null)
                    return (DNEFillStyle)Enum.Parse(m_SelEnumType, cmbSelEnum.SelectedItem.ToString());
                else
                    return 0;
            }
            set
        ...
        public override string GetPropertyEnumType() { return "DNEFillStyle"; }

        public override void SetValue(uint propertyId, object value)
        {
            if (RegPropertyID == propertyId)
                RegEnumVal = (DNEFillStyle)value;
            if (SelPropertyID == propertyId)
                SelEnumVal = (DNEFillStyle)value;
            SetStateValueByProperty(propertyId, (DNEFillStyle)value);
        }
    ...
    public class CtrlObjStatePropertyDataHandlerEFillStyle : CtrlObjStatePropertyDataHandler<DNEFillStyle>
    {

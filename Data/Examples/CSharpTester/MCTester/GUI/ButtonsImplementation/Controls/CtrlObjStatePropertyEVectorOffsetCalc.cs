...
{
    public partial class CtrlObjStatePropertyEVectorOffsetCalc : CtrlObjStatePropertyDataHandlerEVectorOffsetCalc
    {
        private Type m_RegEnumType = null;
        private Type m_SelEnumType = null;
        private string m_EnumType = null;

        public CtrlObjStatePropertyEVectorOffsetCalc()
        {
            InitializeComponent();
            m_InitValue = DNEVectorOffsetCalc._EVOC_PARALLEL_DISTANCE;
            SetEnumList();
        }
        ...
        private void SetEnumList()
        {
            ...
            Type enumType = typeof(DNEVectorOffsetCalc);
        ...
        public DNEVectorOffsetCalc RegEnumVal
        {
            get
            {
                if (cmbRegEnum.SelectedItem != null)
                    return (DNEVectorOffsetCalc)Enum.Parse(m_RegEnumType, cmbRegEnum.SelectedItem.ToString());
                else
                    return 0;
            }
            set
        ...
        public DNEVectorOffsetCalc SelEnumVal
        {
            get
            {
                if (cmbSelEnum.SelectedItem != null)
                    return (DNEVectorOffsetCalc)Enum.Parse(m_SelEnumType, cmbSelEnum.SelectedItem.ToString());
                else
                    return 0;
            }
            set
        ...
        public override string GetPropertyEnumType() { return "DNEVectorOffsetCalc"; }

        public override void SetValue(uint propertyId, object value)
        {
            if (RegPropertyID == propertyId)
                RegEnumVal = (DNEVectorOffsetCalc)value;
            if (SelPropertyID == propertyId)
                SelEnumVal = (DNEVectorOffsetCalc)value;
            SetStateValueByProperty(propertyId, (DNEVectorOffsetCalc)value);
        }
    ...
    public class CtrlObjStatePropertyDataHandlerEVectorOffsetCalc : CtrlObjStatePropertyDataHandler<DNEVectorOffsetCalc>
    {

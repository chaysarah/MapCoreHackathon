...
{
    public partial class CtrlObjStatePropertyEOffsetOrientation : CtrlObjStatePropertyDataHandlerEOffsetOrientation
    {
        private Type m_RegEnumType = null;
        private Type m_SelEnumType = null;
        private string m_EnumType = null;

        public CtrlObjStatePropertyEOffsetOrientation()
        {
            InitializeComponent();
            m_InitValue = DNEOffsetOrientation._EOO_RELATIVE_TO_PARENT_ROTATION;
            SetEnumList();
        }
        ...
        private void SetEnumList()
        {
            ...
            Type enumType = typeof(DNEOffsetOrientation);
        ...
        public DNEOffsetOrientation RegEnumVal
        {
            get
            {
                if (cmbRegEnum.SelectedItem != null)
                    return (DNEOffsetOrientation)Enum.Parse(m_RegEnumType, cmbRegEnum.SelectedItem.ToString());
                else
                    return 0;
            }
            set
        ...
        public DNEOffsetOrientation SelEnumVal
        {
            get
            {
                if (cmbSelEnum.SelectedItem != null)
                    return (DNEOffsetOrientation)Enum.Parse(m_SelEnumType, cmbSelEnum.SelectedItem.ToString());
                else
                    return 0;
            }
            set
        ...
        public override string GetPropertyEnumType() { return "DNEOffsetOrientation"; }

        public override void SetValue(uint propertyId, object value)
        {
            if (RegPropertyID == propertyId)
                RegEnumVal = (DNEOffsetOrientation)value;
            if (SelPropertyID == propertyId)
                SelEnumVal = (DNEOffsetOrientation)value;
            SetStateValueByProperty(propertyId, (DNEOffsetOrientation)value);
        }
    ...
    public class CtrlObjStatePropertyDataHandlerEOffsetOrientation : CtrlObjStatePropertyDataHandler<DNEOffsetOrientation>
    {

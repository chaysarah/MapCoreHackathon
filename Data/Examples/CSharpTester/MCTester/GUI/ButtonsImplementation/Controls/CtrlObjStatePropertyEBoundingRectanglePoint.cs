...
{
    public partial class CtrlObjStatePropertyEBoundingRectanglePoint : CtrlObjStatePropertyDataHandlerEBoundingRectanglePoint
    {
        private Type m_RegEnumType = null;
        private Type m_SelEnumType = null;
        private string m_EnumType = null;

        public CtrlObjStatePropertyEBoundingRectanglePoint()
        {
            InitializeComponent();
            m_InitValue = DNEBoundingRectanglePoint._EBRP_CENTER;
            SetEnumList();
        }
        ...
        private void SetEnumList()
        {
            ...
            Type enumType = typeof(DNEBoundingRectanglePoint);
        ...
        public DNEBoundingRectanglePoint RegEnumVal
        {
            get
            {
                if (cmbRegEnum.SelectedItem != null)
                    return (DNEBoundingRectanglePoint)Enum.Parse(m_RegEnumType, cmbRegEnum.SelectedItem.ToString());
                else
                    return 0;
            }
            set
        ...
        public DNEBoundingRectanglePoint SelEnumVal
        {
            get
            {
                if (cmbSelEnum.SelectedItem != null)
                    return (DNEBoundingRectanglePoint)Enum.Parse(m_SelEnumType, cmbSelEnum.SelectedItem.ToString());
                else
                    return 0;
            }
            set
        ...
        public override string GetPropertyEnumType() { return "DNEBoundingRectanglePoint"; }

        public override void SetValue(uint propertyId, object value)
        {
            if (RegPropertyID == propertyId)
                RegEnumVal = (DNEBoundingRectanglePoint)value;
            if (SelPropertyID == propertyId)
                SelEnumVal = (DNEBoundingRectanglePoint)value;
            SetStateValueByProperty(propertyId, (DNEBoundingRectanglePoint)value);
        }
    ...
    public class CtrlObjStatePropertyDataHandlerEBoundingRectanglePoint : CtrlObjStatePropertyDataHandler<DNEBoundingRectanglePoint>
    {

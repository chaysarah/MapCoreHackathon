...
{
    public partial class CtrlObjStatePropertyEActionOptions : CtrlObjStatePropertyDataHandlerEActionOptions
    {
        private Type m_RegEnumType = null;
        private Type m_SelEnumType = null;
        private string m_EnumType = null;

        public CtrlObjStatePropertyEActionOptions()
        {
            InitializeComponent();
            m_InitValue = DNEActionOptions._EAO_USE_SELECTOR;
            SetEnumList();
        }
        ...
        private void SetEnumList()
        {
            ...
            Type enumType = typeof(DNEActionOptions);
        ...
        public DNEActionOptions RegEnumVal
        {
            get
            {
                if (cmbRegEnum.SelectedItem != null)
                    return (DNEActionOptions)Enum.Parse(m_RegEnumType, cmbRegEnum.SelectedItem.ToString());
                else
                    return 0;
            }
            set
        ...
        public DNEActionOptions SelEnumVal
        {
            get
            {
                if (cmbSelEnum.SelectedItem != null)
                    return (DNEActionOptions)Enum.Parse(m_SelEnumType, cmbSelEnum.SelectedItem.ToString());
                else
                    return 0;
            }
            set
        ...
        public override string GetPropertyEnumType() { return "DNEActionOptions"; }

        public override void SetValue(uint propertyId, object value)
        {
            if (RegPropertyID == propertyId)
                RegEnumVal = (DNEActionOptions)value;
            if (SelPropertyID == propertyId)
                SelEnumVal = (DNEActionOptions)value;
            SetStateValueByProperty(propertyId, (DNEActionOptions)value);
        }
    ...
    public class CtrlObjStatePropertyDataHandlerEActionOptions : CtrlObjStatePropertyDataHandler<DNEActionOptions>
    {

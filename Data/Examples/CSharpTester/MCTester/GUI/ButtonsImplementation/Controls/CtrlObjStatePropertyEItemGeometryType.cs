...
{
    public partial class CtrlObjStatePropertyEItemGeometryType : CtrlObjStatePropertyDataHandlerEItemGeometryType
    {
        private Type m_RegEnumType = null;
        private Type m_SelEnumType = null;
        private string m_EnumType = null;

        public CtrlObjStatePropertyEItemGeometryType()
        {
            InitializeComponent();
            m_InitValue = DNEItemGeometryType._EGT_GEOMETRIC_IN_VIEWPORT;
            SetEnumList();
        }
        ...
        private void SetEnumList()
        {
            ...
            Type enumType = typeof(DNEItemGeometryType);
        ...
        public DNEItemGeometryType RegEnumVal
        {
            get
            {
                if (cmbRegEnum.SelectedItem != null)
                    return (DNEItemGeometryType)Enum.Parse(m_RegEnumType, cmbRegEnum.SelectedItem.ToString());
                else
                    return 0;
            }
            set
        ...
        public DNEItemGeometryType SelEnumVal
        {
            get
            {
                if (cmbSelEnum.SelectedItem != null)
                    return (DNEItemGeometryType)Enum.Parse(m_SelEnumType, cmbSelEnum.SelectedItem.ToString());
                else
                    return 0;
            }
            set
        ...
        public override string GetPropertyEnumType() { return "DNEItemGeometryType"; }

        public override void SetValue(uint propertyId, object value)
        {
            if (RegPropertyID == propertyId)
                RegEnumVal = (DNEItemGeometryType)value;
            if (SelPropertyID == propertyId)
                SelEnumVal = (DNEItemGeometryType)value;
            SetStateValueByProperty(propertyId, (DNEItemGeometryType)value);
        }
    ...
    public class CtrlObjStatePropertyDataHandlerEItemGeometryType : CtrlObjStatePropertyDataHandler<DNEItemGeometryType>
    {

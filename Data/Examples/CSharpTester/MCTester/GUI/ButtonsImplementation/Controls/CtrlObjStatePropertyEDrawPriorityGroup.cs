...
{
    public partial class CtrlObjStatePropertyEDrawPriorityGroup : CtrlObjStatePropertyDataHandlerEDrawPriorityGroup
    {
        private Type m_RegEnumType = null;
        private Type m_SelEnumType = null;
        private string m_EnumType = null;
        //private DNEDrawPriorityGroup m_InitValue = DNEDrawPriorityGroup._EDPG_REGULAR;

        public CtrlObjStatePropertyEDrawPriorityGroup()
        {
            InitializeComponent();
            m_InitValue = DNEDrawPriorityGroup._EDPG_REGULAR;
            SetEnumList();
        }
        ...
        private void SetEnumList()
        {
            ...
            Type enumType = typeof(DNEDrawPriorityGroup);
        ...
        public DNEDrawPriorityGroup RegEnumVal
        {
            get
            {
                if (cmbRegEnum.SelectedItem != null)
                    return (DNEDrawPriorityGroup)Enum.Parse(m_RegEnumType, cmbRegEnum.SelectedItem.ToString());
                else
                    return 0;
            }
            set
        ...
        public DNEDrawPriorityGroup SelEnumVal
        {
            get
            {
                if (cmbSelEnum.SelectedItem != null)
                    return (DNEDrawPriorityGroup)Enum.Parse(m_SelEnumType, cmbSelEnum.SelectedItem.ToString());
                else
                    return 0;
            }
            set
        ...
        public override string GetPropertyEnumType() { return "DNEDrawPriorityGroup"; }

        public override void SetValue(uint propertyId, object value)
        {
            if (RegPropertyID == propertyId)
                RegEnumVal = (DNEDrawPriorityGroup)value;
            if (SelPropertyID == propertyId)
                SelEnumVal = (DNEDrawPriorityGroup)value;
            SetStateValueByProperty(propertyId, (DNEDrawPriorityGroup)value);
        }
    ...
    public class CtrlObjStatePropertyDataHandlerEDrawPriorityGroup : CtrlObjStatePropertyDataHandler<DNEDrawPriorityGroup>
    {

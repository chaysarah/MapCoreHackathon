...
{
    public partial class CtrlObjStatePropertyEParticleEffectState : CtrlObjStatePropertyDataHandlerEParticleEffectState
    {
        private Type m_RegEnumType = null;
        private Type m_SelEnumType = null;
        private string m_EnumType = null;

        public CtrlObjStatePropertyEParticleEffectState()
        {
            InitializeComponent();
            m_InitValue = DNEParticleEffectState._ES_PAUSED;
            SetEnumList();
        }
        ...
        private void SetEnumList()
        {
            ...
            Type enumType = typeof(DNEParticleEffectState);
        ...
        public DNEParticleEffectState RegEnumVal
        {
            get
            {
                if (cmbRegEnum.SelectedItem != null)
                    return (DNEParticleEffectState)Enum.Parse(m_RegEnumType, cmbRegEnum.SelectedItem.ToString());
                else
                    return 0;
            }
            set
        ...
        public DNEParticleEffectState SelEnumVal
        {
            get
            {
                if (cmbSelEnum.SelectedItem != null)
                    return (DNEParticleEffectState)Enum.Parse(m_SelEnumType, cmbSelEnum.SelectedItem.ToString());
                else
                    return 0;
            }
            set
        ...
        public override string GetPropertyEnumType() { return "DNEParticleEffectState"; }

        public override void SetValue(uint propertyId, object value)
        {
            if (RegPropertyID == propertyId)
                RegEnumVal = (DNEParticleEffectState)value;
            if (SelPropertyID == propertyId)
                SelEnumVal = (DNEParticleEffectState)value;
            SetStateValueByProperty(propertyId, (DNEParticleEffectState)value);
        }
    ...
    public class CtrlObjStatePropertyDataHandlerEParticleEffectState : CtrlObjStatePropertyDataHandler<DNEParticleEffectState>
    {

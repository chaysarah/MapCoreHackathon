...
{
    public partial class CtrlObjStatePropertyDictionaryTraversabilityColor : CtrlObjStatePropertyDictionaryDataHandlerTraversabilityColor
    {
        Array arrEnumValues;
        List<DNEPointTraversability> listEnumValues;
        Type enumKey = typeof(DNEPointTraversability);
        
        DNSByteBool currState = false;
        
        DNEPointTraversability selectedKey;

        public CtrlObjStatePropertyDictionaryTraversabilityColor()
        {
            InitializeComponent();
            SetEnumList();
            ...
            selectedKey = DNEPointTraversability._EPT_TRAVERSABLE;

        }

        public DNEPointTraversability EnumKey
        {
            get
            {
                if (cbRegEnumColor.SelectedItem != null)
                    return (DNEPointTraversability)Enum.Parse(enumKey, cbRegEnumColor.SelectedItem.ToString());
                else
                    return 0;
            }
            set
        ...
        public void SetEnumList()
        {
            ...
            listEnumValues = new List<DNEPointTraversability>();
            foreach (DNEPointTraversability value in arrEnumValues)
                if (value != DNEPointTraversability._EPT_NUM)
        ...
        private void SaveCurrentColor(DNEPointTraversability currKey, DNSMcBColor currColor, DNSByteBool state)
        {
            if (currKey == DNEPointTraversability._EPT_NUM)
                return;
        ...
        private void cbRegEnumColor_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            SaveCurrentKey();
            RemoveAllStates();
            selectedKey = (DNEPointTraversability)arrEnumValues.GetValue(cbRegEnumColor.SelectedIndex);
            LoadRegularValue();
        ...
        public new void Load(delegateLoadDic func)
        {
            ...
            LoadStatesByKey(DNEPointTraversability._EPT_TRAVERSABLE);
            SelectFirstTab();
        ...
        private new void LoadStatesByKey(DNEPointTraversability selectedSelKey)
        {
    ...
    public class CtrlObjStatePropertyDictionaryDataHandlerTraversabilityColor : CtrlObjStatePropertyDictionaryDataHandler<DNEPointTraversability, DNSMcBColor> { }
}

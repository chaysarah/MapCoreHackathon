...
{
    public partial class CtrlObjStatePropertyENeverUpsideDownMode : CtrlObjStatePropertyDataHandlerENeverUpsideDownMode
    {
        private Type m_RegEnumType = null;
        private Type m_SelEnumType = null;
        private string m_EnumType = null;

        public CtrlObjStatePropertyENeverUpsideDownMode()
        {
            InitializeComponent();
            m_InitValue = DNENeverUpsideDownMode._ENUDM_NONE;
            SetEnumList();
        }
        ...
        private void SetEnumList()
        {
            ...
            Type enumType = typeof(DNENeverUpsideDownMode);
        ...
        public DNENeverUpsideDownMode RegEnumVal
        {
            get
            {
                if (cmbRegEnum.SelectedItem != null)
                    return (DNENeverUpsideDownMode)Enum.Parse(m_RegEnumType, cmbRegEnum.SelectedItem.ToString());
                else
                    return 0;
            }
            set
        ...
        public DNENeverUpsideDownMode SelEnumVal
        {
            get
            {
                if (cmbSelEnum.SelectedItem != null)
                    return (DNENeverUpsideDownMode)Enum.Parse(m_SelEnumType, cmbSelEnum.SelectedItem.ToString());
                else
                    return 0;
            }
            set
        ...
        public override string GetPropertyEnumType() { return "DNENeverUpsideDownMode"; }

        public override void SetValue(uint propertyId, object value)
        {
            if (RegPropertyID == propertyId)
                RegEnumVal = (DNENeverUpsideDownMode)value;
            if (SelPropertyID == propertyId)
                SelEnumVal = (DNENeverUpsideDownMode)value;
            SetStateValueByProperty(propertyId, (DNENeverUpsideDownMode)value);
        }
    ...
    public class CtrlObjStatePropertyDataHandlerENeverUpsideDownMode : CtrlObjStatePropertyDataHandler<DNENeverUpsideDownMode>
    {
